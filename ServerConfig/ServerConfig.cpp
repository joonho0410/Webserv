...
void ServerConfig::_M_check_serverName(std::vector<struct server_config_struct >::iterator &_temp)
{
    ...
    if (search == _temp->key_and_value.end()) {
        ...
        std::vector<std::string>::iterator begin = search->second.begin();
        std::vector<std::string>::iterator end = search->second.end();
        for (; begin != end; ++begin){
            if ((*begin).compare("localhost") == 0){
                *begin = "127.0.0.1";
            }
        }
    }
...
void ServerConfig::_M_check_listen(std::vector<struct server_config_struct >::iterator _temp)
{
    ...
    else if (search->second.size() == 2 && search->second[1].compare("default_server") == 0) {
