...
{
    std::vector<struct config_struct_server>::iterator begin = _m_server_config_set.begin();
    std::vector<struct config_struct_server>::iterator end = _m_server_config_set.end();
...
struct config_struct_server
ServerEngine::_M_find_location_block(struct config_struct_server &_server_block, std::string &_url)
{
    std::cout << "========== find location block =========== " << std::endl;
    size_t pos = 0;
    std::string str;
    std::string temp;
    struct config_struct_server ret;
...
void ServerEngine::set_config_set(std::vector<struct config_struct_server> _config)
{
    _m_server_config_set = _config;
}

kqueue_data* ServerEngine::_M_make_udata(int _current_fd, bool _is_cgi = false)
{
    kqueue_data* data = new kqueue_data();
    data->is_cgi = _is_cgi;
    data->requested_fd = _current_fd;
    data->response.clear();
    data->error_flag = 0;
    data->fd[0] = -1;
    data->fd[1] = -1;
...
Request ServerEngine::_M_make_request(struct kevent* _curr_event)
{
    Request req;
    char buf[1024];
    ...
    {
        std::cout << _curr_event->ident << " read event " << std::endl;
        size_t len;
        len = 0;
        std::string temp;
        temp.clear();
        ...
        while (n == 1023)
        {
            n = read(_curr_event->ident, buf, sizeof(buf) - 1);
            if (n < 0)
            {
...
void ServerEngine::_M_check_request(Request& req)
{
    ...
    std::string host = *(req.get_map().find("Host")->second.begin());
    struct config_struct_server serv_temp;
    struct config_struct_server loca_temp;
...
void ServerEngine::start_kqueue()
{
    ...
    for (int i = 0; i != _m_server_socket.size(); i++)
        _M_change_events(_m_change_list, _m_server_socket[i], EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, NULL);
        ...
        for (int i = 0; i < new_events; ++i)
        {
            ...
            else if (curr_event->filter == EVFILT_READ)
            {
                ...
                if (std::find(_m_server_socket.begin(), _m_server_socket.end(), curr_event->ident) != _m_server_socket.end())
                {
                    ...
                    _M_change_events(_m_change_list, client_socket, EVFILT_READ , EV_ADD | EV_ONESHOT, 0, 0, NULL);
                    _m_clients[client_socket] = "";
                }

                /* if client socket read contents */
                else if (_m_clients.find(curr_event->ident)!= _m_clients.end())
                {
                    std::cout << curr_event->ident <<" : READ EVENT" <<std::endl;
                    // _m_clients[curr_event->ident] += req.get_buf();
                    // std::cout << "received data from " << curr_event->ident << ": " << _m_clients[curr_event->ident] ;
                    // _m_clients[curr_event->ident].clear();

                    /* need parsing for what client wants */
                    // case 1 -> return page file
                    // case 2 -> do cgi and return result
                    ...
                    if (curr_event->udata == NULL)
                    {
                        ...
                        {
                            std::string temp;
                            temp.clear();                                temp = *(loca_temp.key_and_value.find("client_max_body_size")->second.begin());
                            client_body_size = std::atoi(temp.c_str());
                            std::cout << "========= client body size ========== " << std::endl;
                            std::cout << client_body_size << std::endl;
                            req.check_body_size(client_body_size);
